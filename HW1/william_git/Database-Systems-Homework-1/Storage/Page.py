...
class PageHeader:
  ...
  def __init__(self, **kwargs):
    ...
    buffer[0:len(packed_header)] = packed_header
  ...
  def usedSpace(self):
    return self.freeSpaceOffset - PageHeader.size
  ...
  def pack(self):
    return PageHeader.binrepr.pack(self.flags, self.tupleSize, self.freeSpaceOffset, self.pageCapacity)
  ...
  def unpack(cls, buffer):
    ...
    if len(values) == 4:
      return cls(buffer=buffer, flags=values[0], tupleSize=values[1], freeSpaceOffset=values[2], pageCapacity=values[3])
...
class Page(BytesIO):
  ...
  def getTuple(self, tupleId):
    #Asked for a tuple that is to the right of the freeSpaceOffset
    if self.header.freeSpaceOffset <= (tupleId.tupleIndex * self.header.tupleSize + self.header.headerSize()):
      return None
    else:
      start = tupleId.tupleIndex * self.header.tupleSize + self.header.headerSize()
  ...
  def putTuple(self, tupleId, tupleData):
    if self.header and tupleId and tupleData:
      self.setDirty(True)
      start = self.header.tupleSize * tupleId.tupleIndex + self.header.headerSize()
      end = start + self.header.tupleSize
      self.getbuffer()[start:end] = tupleData

  # Adds a packed tuple to the page. Returns the tuple id of the newly added tuple.
  def insertTuple(self, tupleData):
    if self.header and tupleData and self.header.hasFreeTuple():
      self.setDirty(True)
  ...
  def clearTuple(self, tupleId):
    if self.header and tupleId:
      ...
      self.getbuffer()[start:end] = self.schema.pack(self.schema.default())
  ...
  def deleteTuple(self, tupleId):
    if self.header and tupleId:
      ...
      shiftAmount = self.header.freeSpaceOffset - end
      self.getbuffer()[start:start+shiftAmount] = self.getbuffer()[end:end+shiftAmount]
  ...
  def unpack(cls, pageId, buffer):
    header = PageHeader.unpack(buffer)
